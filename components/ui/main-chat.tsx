...
export function MainChat({ onNewChat }: MainChatProps) {
  const { messages, input, handleInputChange, handleSubmit, isLoading, stop, setInput, setMessages } = useChat({
    api: '/api/chat',
    onFinish: (message) => {
      // DEBUG: Log when server response is complete
      console.log('✅ [MainChat] Server response complete:', {
        id: message.id,
    ...
    onError: (error) => {
      // DEBUG: Log any streaming errors
      console.error('❌ [MainChat] Streaming error:', error);
    }
  })
  ...
  useEffect(() => {
    console.log('🔄 [MainChat] Messages state updated:', {
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-gray-950 min-h-0">
      ...
      {/* Error Display */}
      {error && (
        <div className="flex-shrink-0 p-4 bg-red-50 border-t border-red-200 dark:bg-red-900/20 dark:border-red-800">
